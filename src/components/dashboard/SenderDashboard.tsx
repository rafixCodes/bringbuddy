import { Package, Search, ShoppingBag, ArrowRight, Clock, CheckCircle2, Truck, MapPin, ChevronRight, Star, Shield, Zap } from 'lucide-react'
import { Button } from '../ui'
import { AuthNavbar } from '../AuthNavbar'
import { useRouter } from '../../lib/router'
import { TRAVELERS, TRIPS } from '../../data/prototype'
...
function DeliveryCard({ d }: { d: typeof ACTIVE_DELIVERIES[0] }) {
  const { navigate, setOrderDraft } = useRouter()
...
const SENDER_STATUS: Record<string, { label: string; cls: string }> = {
...
export function SenderDashboard() {
  const { navigate, user, setOrderDraft, activeOrder } = useRouter()
      ...
      <main className="max-w-[1280px] mx-auto px-6 lg:px-10 pt-28 pb-20">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10 animate-[bb-rise_0.45s_cubic-bezier(0.22,1,0.36,1)_both]">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              ...
              onClick={() => { setOrderDraft({}); navigate('order-new') }}
            >
              Create Order
            </Button>
            <Button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            {/* Your active order (reflects live lifecycle state) */}
            ...
            {activeOrder?.status === 'completed' && (
              <div>
                <h2 className="text-[18px] font-bold text-ink mb-4">Recent Completed</h2>
                <div className="rounded-[16px] border border-border bg-white p-5 flex items-start gap-4">
                  <div className="w-10 h-10 rounded-[10px] bg-success-light flex items-center justify-center shrink-0">
                    <CheckCircle2 size={20} className="text-success" />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[14px] font-semibold text-ink">{activeOrder.from} → {activeOrder.to}</p>
                      <span className="text-[11px] font-semibold text-success bg-success-light rounded-full px-2 py-0.5">Completed</span>
                    </div>
                    <p className="text-[13px] text-ink-muted mt-0.5">{activeOrder.weightKg} kg · Delivered {activeOrder.travelDate}</p>
                    <button onClick={() => navigate('order-hub')} className="mt-3 text-[13px] text-primary font-medium flex items-center gap-1 hover:gap-2 transition-all">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="rounded-[16px] border border-border bg-white overflow-hidden">
              ...
              {[
                ...
                { icon: <Package size={15} className="text-primary" />, label: 'New delivery request', action: () => { setOrderDraft({}); navigate('order-new') } },
                { icon: <ShoppingBag size={15} className="text-coral" />, label: 'Browse Marketplace', action: () => navigate('marketplace') },
              ].map(item => (
                <button
                  key={item.label}
                  onClick={item.action}
                  className="flex w-full items-center gap-3 px-5 py-3 text-[13px] text-ink-secondary hover:bg-divider hover:text-ink transition-colors"
                >
