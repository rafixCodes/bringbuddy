...
type SelectedMode = 'sender' | 'traveler' | null

export function ModeSelection() {
  const { navigate, user, setUser, switchMode } = useRouter()
  const [selected, setSelected] = useState<SelectedMode>(null)

  function handleContinue() {
    if (!selected) return
    if (user) {
      setUser({ ...user, mode: selected })
      switchMode(selected)
    }
            ...
            {[
              {
                ...
                cta: 'Start as Sender',
                accentClass: 'from-primary/10 to-primary-light',
              ...
              },
              {
                ...
                accentClass: 'from-coral-light to-coral/10',
