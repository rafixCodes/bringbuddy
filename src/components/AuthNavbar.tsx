...
import { useRouter, type UserMode } from '../lib/router'
...
function ModeToggle({ mode, onSwitch }: { mode: UserMode; onSwitch: (m: UserMode) => void }) {
  const other = mode === 'sender' ? 'traveler' : 'sender'
  const Icon = mode === 'sender' ? Package : Plane
  const label = mode === 'sender' ? 'Sender' : 'Traveler'
...
function ProfileDropdown({ onClose }: { onClose: () => void }) {
  const { user, navigate, setUser, switchMode, activeOrder } = useRouter()
  ...
  function handleSwitch(m: UserMode) {
    switchMode(m)
    if (m === 'traveler' && activeOrder?.status === 'pending') {
      toast({ tone: 'info', title: '🔔 New delivery request', message: `A ${activeOrder.weightKg} kg parcel is waiting for your response.` })
    } else {
    ...
    <div className="absolute right-0 top-full mt-2 w-64 rounded-[14px] bg-white shadow-[var(--shadow-e3)] border border-border overflow-hidden z-50 animate-[bb-rise_0.2s_cubic-bezier(0.22,1,0.36,1)_both]">
      ...
      <div className="px-4 py-3 border-b border-border">
        ...
        <div className="flex flex-col gap-1">
          {(['sender', 'traveler'] as const).map(m => (
            <button
              key={m}
              onClick={() => handleSwitch(m)}
              className={`flex items-center gap-2.5 rounded-[8px] px-3 py-2 text-[13px] font-medium transition-all ${
                user?.mode === m ? (m === 'sender' ? 'bg-primary-light text-primary' : 'bg-coral-light text-coral') : 'text-ink-secondary hover:bg-divider'
              }`}
            >
              {m === 'sender' ? <Package size={14} /> : <Plane size={14} />}
              {m === 'sender' ? 'Sender' : 'Traveler'}
              {user?.mode === m && <span className="ml-auto text-[10px] font-bold opacity-60">ACTIVE</span>}
            </button>
          ))}
        </div>
      ...
      <div className="py-1.5">
        {[
          ...
          ...(user?.mode === 'traveler' ? [{ icon: <ShoppingBag size={14} />, label: 'Earnings', action: () => { navigate('earnings'); onClose() } }] : []),
        ].map(item => (
          <button
            key={item.label}
            onClick={item.action}
...
function NotificationPanel({ mode }: { mode?: UserMode }) {
  const events = mode === 'traveler'
    ? [
        { time: '2:31 PM', text: 'New delivery request from Alex Johnson', icon: '🔔' },
        { time: '2:40 PM', text: 'Sender secured payment in escrow', icon: '🛡️' },
        { time: 'Yesterday', text: 'You earned a new 5-star review', icon: '⭐' },
      ]
    : [
        { time: '2:31 PM', text: 'Aisha accepted your delivery request', icon: '✅' },
        { time: '2:40 PM', text: 'Payment secured in escrow', icon: '🛡️' },
        { time: 'Yesterday', text: 'Delivery completed · Order #BB-1041', icon: '📦' },
      ]
  return (
...
export function AuthNavbar() {
  const { navigate, user, switchMode, activeOrder } = useRouter()
  const { toast } = useToast()
  ...
  function handleModeSwitch(m: UserMode) {
    switchMode(m)
    if (m === 'traveler' && activeOrder?.status === 'pending') {
      toast({ tone: 'info', title: '🔔 New delivery request', message: `A ${activeOrder.weightKg} kg parcel is waiting for your response.` })
    } else {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
      ...
      <nav className="mx-auto flex h-16 max-w-[1280px] items-center justify-between px-6 lg:px-10 gap-4">
        <button onClick={() => navigate(user?.mode === 'traveler' ? 'traveler-dashboard' : 'sender-dashboard')} className="rounded-lg transition-opacity hover:opacity-80">
          <Logo />
        </button>
        ...
        <div className="hidden md:flex items-center gap-1 flex-1 ml-4">
          {(user?.mode === 'traveler' ? [
            { label: 'Dashboard', page: 'traveler-dashboard' as const },
            { label: 'My Trips', page: 'my-trips' as const },
            { label: 'Marketplace', page: 'marketplace' as const },
            { label: 'Earnings', page: 'earnings' as const },
          ] : [
            { label: 'Dashboard', page: 'sender-dashboard' as const },
            { label: 'Find a Trip', page: 'trip-search' as const },
            { label: 'Marketplace', page: 'marketplace' as const },
            { label: 'My Orders', page: 'order-history' as const },
          ]).map(item => (
            <button
              key={item.label}
        ...
        <div className="flex items-center gap-3">
          {/* Mode toggle */}
          {user && <ModeToggle mode={user.mode} onSwitch={handleModeSwitch} />}
