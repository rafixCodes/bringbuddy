...
export function Login() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    // Prototype: treat any well-formed input as success
    if (email && password.length >= 6) {
      setState('success')
      setUser({ name: 'Alex Johnson', email, mode: 'sender', verificationStatus: 'none' })
      toast({ tone: 'success', title: 'Login successful', message: 'Welcome back to BringBuddy.' })
      setTimeout(() => navigate('sender-dashboard'), 600)
    } else {
      setState('error')
