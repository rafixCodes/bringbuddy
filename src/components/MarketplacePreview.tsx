...
const listings: TravelerListing[] = [
...
function TravelerCard({ t, delay }: { t: TravelerListing; delay: string }) {
  return (
    <div className={`bb-reveal ${delay} group flex h-full flex-col rounded-[12px] border border-border bg-white p-5 transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/30 hover:shadow-[var(--shadow-e2)]`}>
      ...
      <Button variant="secondary" className="mt-4 w-full justify-center group-hover:border-primary/40" trailingIcon={<ArrowRight size={16} />}>
        View Trip
      </Button>
    </div>
  )
    ...
    <section className="relative overflow-hidden py-24">
      <div className="mx-auto max-w-[1280px] px-6 lg:px-10">
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {listings.map((t, i) => (
            <TravelerCard key={t.name} t={t} delay={`bb-delay-${i + 1}`} />
          ))}
        </div>
      </div>
    </section>
