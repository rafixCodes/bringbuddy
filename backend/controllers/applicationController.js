...
const acceptApplication = async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const order = await Order.findById(application.order).session(
        session
      );
      ...
      if (!traveler) {
        throw createHttpError('Traveler account not found', 404);
      }
      ...
      const trip = await Trip.findById(application.trip).session(
        session
      );

      if (!trip) {
        throw createHttpError('Traveler trip not found', 404);
      }

      if (
        trip.traveler.toString() !== traveler._id.toString() ||
      ...
      order.timeline.push({
        status: 'accepted',
        note: 'Traveler selected through marketplace application',
      });
